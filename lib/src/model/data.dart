...
Future<void> addReview({String restaurantId, Review review}) {
  ...
  return FirebaseFirestore.instance.runTransaction((Transaction transaction) {
    return transaction
      ...
      transaction.update(
          restaurant, {'numRatings': newRatings, 'avgRating': newAverage});
...
Stream<QuerySnapshot> loadFilteredRestaurants(Filter filter) {
  Query filterCollection = FirebaseFirestore.instance.collection('restaurants');
  if (filter.category != null) {
    filterCollection =
        filterCollection.where('category', isEqualTo: filter.category);
  }
  if (filter.city != null) {
    filterCollection = filterCollection.where('city', isEqualTo: filter.city);
  }
  if (filter.price != null) {
    filterCollection = filterCollection.where('price', isEqualTo: filter.price);
  }
